...
import json
from Aktuelle_Stand.HELPERS import *

standard_guidline = """
Controllability (C), i.e. the ability to avoid the specific harm or damage through timely reactions of the persons involved
C0: Controllable in general, 
C1: Simply controllable / More than 99 % of the average operating person or other personnel are able to avoid harm, 
C2: Normally controllable / Between 90 % an 99 % of the average operating person or other personnel are able to avoid harm, 
C3: Difficult to control or uncontrollable / Less than 90 % of the average operating person or other personnel are able to avoid harm
NOTE: IF the percentage was unclear / vague to calculate use the normal terms stated first.

NOTE:
- The evaluation of controllability assesses the likelihood that an individual can gain sufficient control over a hazardous event to prevent the specific harm from occurring. It is assumed that the driver is in an appropriate condition to operate the vehicle, has received proper driver training, and is adhering to relevant legal regulations, including necessary precautions to protect other road users.
- If the hazardous event is not related to controlling the vehicle's direction or speed, controllability can be seen as an estimation of the likelihood that the person at risk can either remove themselves or be removed by others from the dangerous situation.
- When considering controllability, note that the person at risk might not be familiar with the operation of the item or may not even realize that a hazardous situation is developing.
-----------------------------------------------------------------------------------------------
The potential severity (S) of the resulting harm or damage:
S0:No injuries, S1:Light and moderate injuries Class, S2:Severe and life-threatening injuries(survival probable), S3:Life-threatening injuries (survival uncertain) / fatal injuries

NOTES:
- S0 is also assigned if the hazard analysis and risk assessment determines that the consequences are clearly limited to material damage, where the damage only occurs to the machinery. 
- To describe the severity, the AIS classification is used.
		...
		AIS 2: Moderate injuries such as deep lacerations, concussions with up to 15 minutes of unconsciousness, uncomplicated fractures of long bones or ribs, etc.
		AIS 3: Severe, non-life-threatening injuries, including skull fractures without brain damage, spinal dislocations below the fourth cervical vertebra without spinal cord injury, multiple rib fractures without paradoxical breathing, etc.
		AIS 4: Serious injuries (life-threatening, but survival is likely) such as concussions with or without skull fractures, resulting in up to 12 hours of unconsciousness, or 	paradoxical breathing.
		AIS 5: Critical injuries (life-threatening, uncertain survival), including spinal fractures below the fourth cervical vertebra with spinal cord damage, intestinal or cardiac tears, unconsciousness lasting more than 12 hours, or intracranial bleeding.
		AIS 6: Extremely critical or fatal injuries, such as fractures of the cervical vertebrae above the third vertebra with spinal cord damage, or severe open wounds in the thoracic or abdominal cavities.
...
NOTES:
- The number of vehicles equipped with the item shall not be considered when estimating the probability of exposure, assuming each vehicle is equipped with the item. This means that the argument “the probability of exposure can be reduced, because the item is not present in every vehicle (as only some vehicles are equipped with the item)” is not valid.
...
- The remaining E1, E2, E3 and E4 levels are assigned for situations that can become hazardous depending on either the duration of a situation (temporal overlap) or the frequency of occurrence of a situation.
	 In the first case the probability of exposure is typically estimated by the proportion of time spent in the considered situation compared to the total operating time, Note that in some cases the total operating time can be the vehicle life-time
-  If the time period in which a failure remains latent is comparable to the time period before the hazardous event can be expected to take place, then the estimation of the probability of exposure considers that time period. Typically this will concern devices that are expected to act on demand, e.g. airbags.
	  => In this case, the probability of exposure is estimated by σ × T where σ is the rate of occurrence of the operational situation and T is the duration during which the failure is not perceived (possibly up to the lifetime of the vehicle). 
Where if (the % of average operating time) was available then normally this is the correlation between it and the table of E:
 E1 -> Not specified
...
-----------------------------------------------------------------------------------------------
NOTES:
- If classification of a given hazard with respect to severity (S), probability of exposure (E) or controllability (C) is difficult to make, it is classified conservatively, i.e. whenever there is a reasonable doubt, a higher S, E or C classification is chosen.
...
- In addition to these four ASILs, the class QM  indicates that quality processes are sufficient to manage the identified risk.
...
def evaluate_hazards(hazards: List[Dict], model: str) -> List[Dict]:
    results = []
    for idx, hazard in enumerate(hazards, start=1):
        messages = [
            ...
            {"role": "user", "content":
                ...
                f"Risk parameters and Guidline to assess and follow: {standard_guidline}.\n"
                "Each object should include the assigned value and rationale for each risk parameter."
                "If the necessary information can not be derived from the guidance, then mark the hazard’s risk as unknown"
                """OUTPUT REQUIREMENTS:
...
def run_risk_assessment(hazards: List[dict], model: str = "openai:gpt-4o") -> List[dict]:
...
if __name__ == "__main__":
    ...
    res = run_risk_assessment(hazards=hazards)
    for idx, thing in enumerate(res):
        print(f"{idx}|{thing["hazard"]}\n  {thing["Severity"]["value"]}|{thing["Exposure"]["value"]}|{thing["Controllability"]["value"]}|{thing["ASIL"]}")
